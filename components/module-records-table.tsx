...
import { useDeleteDynamicRecordMutation } from "@/lib/api/records"
...
export default function ModuleRecordsTable({
    ...
    const [deleteDynamicRecord] = useDeleteDynamicRecordMutation()
    ...
    const RenderFieldValue = ({ data, fieldName }: { data: any; fieldName: string }) => {
        ...
        const isImageFieldName = isImageField(fieldName)
        ...
        // Try to parse stringified JSON (common in some DB setups)
                                ...
                                {Object.entries(row)
                                    ...
                                    .map(([subFieldId, subValue]) => (
                                        <div key={subFieldId} className="flex gap-1 overflow-hidden">
                                            ...
                                            <RenderFieldValue data={row} fieldName={subFieldId} />
                                        </div>
                                    ))}
                            </div>
                        </div>
        ...
        const ImagePreview = ({ url, size = "h-16 w-16" }: { url: string; size?: string }) => (
            <button
                onClick={(e) => {
                    e.stopPropagation();
            ...
            >
                <img
                    src={url}
                    alt="Content"
                    className={`${size} object-cover rounded border bg-white`}
                    onError={(e) => {
        ...
        // Helper to check if a value is "image-like"
        const isImageLike = (v: any): boolean => {
            if (typeof v === 'string') return isImageURL(v) || (v.startsWith('http') && (isImageFieldName || !!v.match(/\.(jpg|jpeg|png|gif|webp|svg|bmp)/i) || v.includes('hostinger') || v.includes('cloudinary') || v.includes('googleusercontent')));
            if (typeof v === 'object' && v !== null) return !!(v.url || v.imageUrl || v.path);
            return false;
        }

        const getUrlFromObj = (v: any): string => {
            if (typeof v === 'string') return v;
            return v?.url || v?.imageUrl || v?.path || JSON.stringify(v);
        }
        ...
        if (Array.isArray(value)) {
            ...
            const imageItems = value.filter(isImageLike);
            if (imageItems.length > 0) {
                return (
                    <div className="flex flex-wrap gap-1.5 mt-1">
                        {imageItems.map((v, idx) => <ImagePreview key={idx} url={getUrlFromObj(v)} size="h-10 w-10" />)}
                    </div>
                )
            }
                ...
                <div className="flex flex-wrap gap-1">
                    {value.map((v, idx) => (
                        <Badge key={idx} variant="outline" className="text-[10px] py-0 px-1">
                            {typeof v === 'object' ? (v?.label || getUrlFromObj(v)) : String(v)}
                        </Badge>
                    ))}
                </div>
            )
        ...
        if (isImageLike(value)) {
            return <ImagePreview url={getUrlFromObj(value)} />
        }

        // Handle single string that might be comma-separated URLs
        if (typeof value === "string" && value.includes(',') && isImageFieldName) {
            const urls = value.split(',').map(u => u.trim()).filter(Boolean);
            if (urls.some(isImageURL)) {
                return (
                    <div className="flex flex-wrap gap-1.5 mt-1">
                        {urls.map((url, idx) => <ImagePreview key={idx} url={url} size="h-10 w-10" />)}
                    </div>
                )
            }
        }
        ...
        if (typeof value === "object") {
            ...
            return <span className="text-[10px] font-mono text-gray-400 truncate max-w-full block">{JSON.stringify(value)}</span>
        }
    ...
    const deleteRecord = async (recordId: string) => {
        try {
            await deleteDynamicRecord(recordId).unwrap()
            toast({
                title: "Success",
