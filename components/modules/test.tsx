...
const RecordsDisplay: React.FC<RecordsDisplayProps> = ({
    ...
    const [, setViewDetailsOpen] = React.useState(false);
    ...
    const [activeFieldFilters, setActiveFieldFilters] = React.useState<
        FieldFilter[]
    >([]);
    const [, setSelectedFieldForAdvancedFilter] =
    ...
    const buildProcessedDataFromRecordData = (
        ...
        const formatDisplayValue = (value: any, type: string, key: string): string => {
            if (value === null || value === undefined || value === "") {
                return "—";
            } else if (key.startsWith("_dynamicRows_")) {
                                    ...
                                    >
                                        <div className="inline-block min-w-max">
                                            <div>
                                                ...
                                                {/* Header Row 3: Sections within Subforms */}
                                                <div className={cn(
                                                    "flex bg-gray-100 border-b border-gray-300 sticky z-10 min-w-max",
                                                    isMergedMode && hierarchyGroups.length > 1 ? "top-[72px]" : "top-10"
                                                )}>
                                                    ...
                                                    {hierarchyGroups.map((formGroup) => (
                                                        <React.Fragment key={formGroup.id}>
                                                            {/* Direct sections placeholder */}
                                                            {formGroup.directSections.map((sec) => (
                                                                <div
                                                                    key={`${formGroup.id}-direct-sec-${sec.id}`}
                                                                    className="h-8 bg-gray-100 border-r border-gray-300"
                                                                    style={{ width: `${getGroupWidth(sec.fields)}px` }}
                                                                />
                                                            ))}
                                                            {/* Sections within subforms */}
                                                            {formGroup.subforms.map((sf) => (
                                                                <React.Fragment key={sf.id}>
                                                                    {sf.sections.map((sec) => (
                                                                        <div
                                                                            key={sec.id}
                                                                            className="h-8 bg-gray-100 flex items-center justify-center text-xs font-semibold text-gray-700 border-r border-gray-300"
                                                                            style={{ width: `${getGroupWidth(sec.fields)}px` }}
                                                                        >
                                                                            {sec.title || ""}
                                                                        </div>
                                                ...
                                                <div className={cn(
                                                    "flex bg-slate-100 border-b border-gray-300 sticky z-10 min-w-max",
                                                    isMergedMode && hierarchyGroups.length > 1 ? "top-[104px]" : "top-[72px]"
                                                )}>
                                                    <div className="w-10 flex-shrink-0" />
                                                    <div className="w-12 flex-shrink-0" />
                                                    <div className="w-20 sm:w-24 flex-shrink-0" />
                                                                                        ...
                                                                                        >
                                                                                            {isEditing ? (
                                                                                                ...
                                                                                                <div className="relative group w-full h-full">
                                                                                                    <div
                                                                                                        className={cn(
                                                                                                            "w-full text-sm text-gray-700 leading-tight py-2",
                                                                                                            isWrapTextEnabled || isExpanded
