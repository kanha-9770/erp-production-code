...
// ── Icons ───────────────────────────────────────────────────────
import {
    Loader2, Plus, Table, Grid, List, Edit, Trash2,
    FolderPlus, Settings, Globe, ExternalLink, Eye,
    FileText, Info, ChevronRight, Lock,
} from "lucide-react";
...
import { usePermissionContext, PermissionGate } from "@/context/PermissionContext";
...
export default function ModuleDashboard() {
    ...
    const isInitializing = !organizationId || modulesLoading || permissionsLoading;
    ...
    const [moduleData, setModuleData] = useState({ name: "", description: "", parentId: "" });
        ...
        if (searchQuery.trim()) {
            ...
            const filterRecursive = (items: FormModule[]): FormModule[] =>
                items
                    ...
                    .filter((m) => m.name.toLowerCase().includes(q) || m.children?.length);
    ...
    // ── Helpers ─────────────────────────────────────────────────────
    const findModuleById = (id: string, list: FormModule[]): FormModule | null => {
        for (const m of list) {
            if (m.id === id) return m;
            if (m.children) {
    ...
    const handleCreateModule = async () => {
        if (!can("create", selectedModule?.id)) {
            toast({ title: "Access Denied", description: "No permission to create modules", variant: "destructive" });
            return;
        }
    ...
    const handleEditModule = async () => {
        ...
        if (!can("update", editingModule.id)) {
            toast({ title: "Access Denied", description: "No permission to edit", variant: "destructive" });
            return;
        }
        if (!moduleData.name.trim()) {
            toast({ title: "Error", description: "Name is required", variant: "destructive" });
    ...
    const handleDeleteModule = async (id: string) => {
        if (!can("delete", id)) {
            toast({ title: "Access Denied", description: "No permission to delete", variant: "destructive" });
            return;
        }
    ...
    const handleCreateForm = async () => {
        if (!selectedModule || !can("create", selectedModule.id)) {
            toast({ title: "Access Denied", description: "Cannot create form here", variant: "destructive" });
            return;
        }
    ...
    const handleEditForm = async () => {
        if (!editingForm || !can("update", editingForm.moduleId, editingForm.id)) {
            toast({ title: "Access Denied", description: "No permission to edit form", variant: "destructive" });
            return;
        }
        if (!formData.name.trim()) {
            toast({ title: "Error", description: "Name is required", variant: "destructive" });
        ...
        setIsSubmitting(true);
        try {
            await updateFormOptimistic(editingForm.id, editingForm.moduleId, formData);
    ...
    const handlePublishForm = async (form: Form) => {
        if (!can("publish", form.moduleId, form.id)) {
            toast({ title: "Access Denied", description: "No permission to publish", variant: "destructive" });
            return;
        }
    ...
    const openEditModule = (mod: FormModule) => {
        if (!can("update", mod.id)) {
            toast({ title: "Access Denied", description: "Cannot edit this module", variant: "destructive" });
            return;
        }
        setEditingModule(mod);
        setModuleData({ name: mod.name, description: mod.description || "", parentId: mod.parentId || "" });
        setIsEditDialogOpen(true);
    };

    const openSubmodule = (mod: FormModule) => {
        if (!can("create", mod.id)) {
            toast({ title: "Access Denied", description: "Cannot create submodule here", variant: "destructive" });
            return;
        }
    ...
    const openEditForm = (form: Form) => {
        if (!can("update", form.moduleId, form.id)) {
            toast({ title: "Access Denied", description: "Cannot edit this form", variant: "destructive" });
            return;
        }
        setEditingForm(form);
        ...
        <div className="flex flex-col h-screen bg-gray-50">
            ...
            <header className="bg-white border-b shadow-sm flex-shrink-0">
                <div className="container px-6 py-4 flex items-center justify-between">
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900">Modules & Forms</h1>
                        <p className="text-sm text-gray-600">Manage your structure</p>
                    </div>
                    ...
                    {selectedModule ? (
                        <div className="space-y-6">
                            ...
                            <div className="flex items-center text-sm text-gray-600">
                                <button onClick={() => setSelectedModule(null)} className="hover:text-gray-900">
                                    Modules
                                </button>
                                {getPathToModule(selectedModule).map((m, i, arr) => (
                                    <React.Fragment key={m.id}>
                                        ...
                                        <button
                                            ...
                                            className={i === arr.length - 1 ? "font-semibold text-gray-900" : "hover:text-gray-900"}
                                        >
                                            {m.name}
                                        </button>
                                    </React.Fragment>
                                ...
                                {selectedForm && (
                                    <>
                                        <ChevronRight className="mx-1 h-4 w-4" />
                                        <span className="font-semibold text-gray-900">{selectedForm.name}</span>
                                    </>
                                )}
                            </div>
                            ...
                            <div className="flex items-start justify-between">
                                    ...
                                    {selectedModule.description && (
                                        <TooltipProvider>
                                            <Tooltip>
                                                ...
                                                <TooltipContent>
                                                    <p className="max-w-xs">{selectedModule.description}</p>
                                                </TooltipContent>
                                            </Tooltip>
                                        </TooltipProvider>
                                    )}
                                ...
                                <div className="flex gap-2 flex-wrap">
                                    <PermissionGate permission="create" moduleId={selectedModule.id}>
                                        <Button variant="outline" onClick={() => openSubmodule(selectedModule)}>
                                            <FolderPlus className="mr-2 h-4 w-4" /> Submodule
                                        </Button>
                                    </PermissionGate>

                                    <PermissionGate permission="update" moduleId={selectedModule.id}>
                                        <Button variant="outline" onClick={() => openEditModule(selectedModule)}>
                                            <Edit className="mr-2 h-4 w-4" /> Edit
                                        </Button>
                                    </PermissionGate>

                                    <PermissionGate permission="delete" moduleId={selectedModule.id}>
                                        <Button variant="outline" onClick={() => handleDeleteModule(selectedModule.id)}>
                                            <Trash2 className="mr-2 h-4 w-4" /> Delete
                                        </Button>
                                    </PermissionGate>

                                    <PermissionGate permissions={["manage", "read"]} moduleId={selectedModule.id}>
                                        <Button asChild>
                            ...
                            <div className="bg-white rounded-lg border shadow-sm">
                                ...
                                {!selectedModule.forms?.length ? (
                                    <div className="py-16 text-center text-gray-500">
                                        ...
                                        <p className="mt-2">Create your first form in this module</p>
                                    </div>
                                ) : viewMode === "table" ? (
                                                        ...
                                                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                            <div className="flex items-center justify-end gap-1">
                                                                {can("read", selectedModule.id, form.id) && (
                                                                    <Button variant="ghost" size="sm" title="View / Fill">
                                                                        <Eye className="h-4 w-4 text-blue-600" />
                                                                    </Button>
                                                                )}

                                                                {can("update", selectedModule.id, form.id) && (
                                                                    <Button asChild variant="ghost" size="sm" title="Edit Builder">
                                                                ...
                                                                {form.isPublished && can("read", selectedModule.id, form.id) && (
                                                                    <Button asChild variant="ghost" size="sm" title="Open Published Form">
                                                                        <NextLink href={`/form/${form.id}`} target="_blank">
                                                                ...
                                                                {can("delete", selectedModule.id, form.id) && (
                                                                    <Button
                                                                        ...
                                                                        onClick={() => handleDeleteForm(form.id, form.moduleId)}
                                                                        title="Delete"
                                                                    >
                    ...
                    <div className="space-y-4 py-4">
                        <div>
                            ...
                            <Input
                                ...
                                onChange={(e) => setModuleData((s) => ({ ...s, name: e.target.value }))}
                        ...
                        <div>
                            ...
                            <Textarea
                                ...
                                onChange={(e) => setModuleData((s) => ({ ...s, description: e.target.value }))}
                        ...
                        <div>
                            ...
                            <select
                                ...
                                onChange={(e) => setModuleData((s) => ({ ...s, parentId: e.target.value }))}
                            >
            ...
            <Dialog open={isSubmoduleDialogOpen} onOpenChange={setIsSubmoduleDialogOpen}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>Under: {parentModuleForSubmodule?.name}</DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4 py-4">
                        <div>
                            <Label>Name</Label>
