...
export default function VerifyOTPForm({ userId, type = "REGISTRATION" }: VerifyOTPFormProps) {
  ...
  const handleOTPChange = (index: number, value: string) => {
    ...
    // clear error message if user modifies input
  ...
  const handlePaste = (index: number, e: React.ClipboardEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const newVal = currentOTP.join('').slice(0,6)
      form.setValue('otp', newVal)
  ...
  const handleResendOTP = async () => {
    ...
    // Clear OTP inputs immediately when user clicks resend
    try {
      form.setValue("otp", "")
    } catch (e) {
      /* ignore */
