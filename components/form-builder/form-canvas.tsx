  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto space-y-4 sm:space-y-6">
        ...
        {/* Rest of your form builder (unchanged) */}
        {(form.sections?.length || topLevelSubforms.length) ? (
          <div className="space-y-6 sm:space-y-10">
            {(form.sections || []).map((section) => (
              <div key={section.id} className="relative">
                ...
                {getSubformsUnderSection(section.id).map((subform: any) => (
                  <div
                    key={subform.id}
                    className="mt-4 sm:mt-6 ml-0 sm:ml-8 pl-3 sm:pl-6 border-l-2 sm:border-l-4 border-blue-200 bg-slate-50/40 rounded-r-lg"
                  >
                    <SubformComponent
                      subform={subform}
                      onUpdateSubform={(updates) => updateSubform(subform.id, updates)}
