...
//                   ...section,
//                   fields: section.fields.map((f: { id: string }) =>
//                     f.id === fieldId ? { ...f, ...updates } : f
//                   ),
//                 }))
//                 const updatedSubforms = (form.subforms || []).map(subform => ({
//                   ...subform,
//                   fields: subform.fields.map((f: { id: string }) =>
//                     f.id === fieldId ? { ...f, ...updates } : f
//                   ),
//                 }))
//                 onFormUpdate({ ...form, sections: updatedSections, subforms: updatedSubforms })
...
export default function FormCanvas({
  ...
  // ────────────────────────────────────────────────
  // IMPORTANT: Real backend save for field updates
  // ────────────────────────────────────────────────
  const updateField = async (fieldId: string, updates: Partial<FormField>) => {
    ...
    const previousForm = { ...form } // for rollback
    onFormUpdate({ ...form, sections: updatedSections, subforms: updatedSubforms })
    ...
    try {
      const response = await fetch(`/api/fields/${fieldId}`, {
        ...
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify(updates), // ← dependentGroups, isDependent, parentFieldId go here
      })
      ...
      const result = await response.json()
      console.log("[FormCanvas] Field updated successfully on server:", result)
        ...
        {selectedField && (
          <FieldSettings
            ...
            onUpdate={async (updates) => {
              ...
              console.log("[FormCanvas → FieldSettings onUpdate] Received updates:", updates)

              // Pass updates to the real save function (which hits backend)
