...
// // Import FieldSettings (adjust path if needed)
...
//   // ────────────────────────────────────────────────
//   // NEW: State for real Field Settings dialog
//   // ────────────────────────────────────────────────
...
//   // ────────────────────────────────────────────────
//   // Open real settings dialog (replaces toast placeholder)
//   // ────────────────────────────────────────────────
//   const openFieldSettings = (field: FormField) => {
...
//   // ────────────────────────────────────────────────
//   // Collect ALL fields from sections + subforms (for parent dropdown)
//   // ────────────────────────────────────────────────
//   const allFormFields: FormField[] = [
//     ...(form.sections || []).flatMap((section) => section.fields || []),
//     ...(form.subforms || []).flatMap((subform) => {
//       const collect = (sf: Subform): FormField[] => [
//         ...(sf.fields || []),
//         ...(sf.childSubforms || []).flatMap(collect),
//       ]
//       return collect(subform)
//     }),
//   ]

//   // Debug log – remove later
//   console.log("All collected fields in FormCanvas:", allFormFields.map(f => ({
//     id: f.id,
...
//         {/* Real Field Settings Dialog – with allFormFields passed */}
//         {selectedField && (
//           <FieldSettings
//             field={selectedField}
//             open={settingsOpen}
...
//             onUpdate={(updates) => {
//               // You can add backend save here later
...
//   // State for Field Settings Dialog
//   const [settingsOpen, setSettingsOpen] = useState(false)
//   const [selectedField, setSelectedField] = useState<FormField | null>(null)
...
//                 className="data-[state=checked]:bg-green-600 data-[state=unchecked]:bg-gray-400"
//               />
//               <Label htmlFor="employee-form-toggle" className="text-sm font-medium cursor-pointer select-none">
//                 Employee Form
//               </Label>
//             </div>
...
//             {isTogglingEmployee && <Loader2 className="h-4 w-4 animate-spin text-muted-foreground" />}
//           </div>
//         </div>
...
//         {selectedField && (
//           <>
//             {console.log("Opening FieldSettings → field:", selectedField.id, "formId:", form.id)}
//             <FieldSettings
//               field={selectedField}
//               open={settingsOpen}
//               onOpenChange={setSettingsOpen}
...
//                 onFormUpdate({ ...form, sections: updatedSections, subforms: updatedSubforms })
//               }}
//               formId={form.id}                    // ← This is now required for API fetch
//             />
//           </>
//         )}
//       </div>
  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        {selectedField && (
          <>
            {console.log("FormCanvas → Passing formId to FieldSettings:", form?.id || "MISSING FORM ID HERE!")}
            <FieldSettings
              field={selectedField}
              open={settingsOpen}
              onOpenChange={setSettingsOpen}
              ...
              formId={form.id}  // ← This line MUST pass a valid string (the form's ID)
            />
          </>
        )}
      </div>
