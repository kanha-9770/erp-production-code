...
/**
 * Product Master — manage the dropdowns used on the product form (Category,
 * Variant, Speed UoM, Sales Channel, Air Flow/Pressure UoM) and register new
 * ones. Same UX as the Purchase / Accounts master managers.
 */
...
import type { MasterOption, MasterType, ProductSubmoduleKey } from "@/lib/product-system/types";
...
const SUBMODULE_LABEL: Record<ProductSubmoduleKey, string> = {
  product: "Product",
};

export function MasterManager() {
  const { ready, masters, addMasterType } = useProduct();
  const [activeKey, setActiveKey] = useState<string>("product_category");
    ...
    <div className="h-full flex flex-col">
      <div className="px-4 sm:px-6 py-4 border-b shrink-0 flex items-start gap-3">
        <div className="min-w-0 flex-1">
          <h1 className="text-lg font-semibold tracking-tight">Product Master</h1>
          <p className="text-sm text-muted-foreground">
            Manage the dropdowns used on the product form — Category, Variant, UoMs and more. You can
            add your own too.
          </p>
        </div>
...
function MasterOptionsPanel({ master }: { master: MasterType }) {
  const { addMasterOption, updateMasterOption, deleteMasterOption, deleteMasterType } = useProduct();
  ...
  return (
    <div className="p-4 sm:p-6 space-y-5">
      ...
      <p className="text-xs text-muted-foreground">
        Inactive options stay on existing products but are hidden from new selections.
      </p>
      ...
      <AlertDialog open={!!deleting} onOpenChange={(o) => !o && setDeleting(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Removing this option won’t change products that already use it, but it will no longer be
    ...
    >
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Create a reusable dropdown (e.g. Machine Series, Material Grade, Region). You can add its
            options right after.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4 py-1">
          <div className="space-y-1.5">
            ...
            <Input
              autoFocus
              placeholder="e.g. Machine Series"
              value={label}
