...
export function OrganizationTree() {
  ...
  const centerView = useCallback(() => {
    ...
    setTransform({
      x: rect.width / 2 - 200, // Roughly center the first node
      y: 80,
      scale: window.innerWidth < 640 ? 0.5 : 0.7, // smaller scale on mobile
    });
  }, []);

  // Center on initial load + resize
  useEffect(() => {
    centerView();
  ...
  const viewWidths = { mobile: "380px", tablet: "720px", desktop: "100%" };
  ...
  return (
    ...
    >
      {/* Dynamic Control Bar – responsive wrapping & sizing */}
      ...
      {/* INFINITE CANVAS AREA – responsive padding */}
      <div
        ref={containerRef}
        onMouseDown={handleMouseDown}
        className={cn(
