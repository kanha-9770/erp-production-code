...
export function PublicFormDialog({
  ...
  // Enhanced visibility checker that supports multi-select conditional
  const isFieldVisibleWithMultiSupport = (fieldId: string): boolean => {
    const currentField = allFields.find((f) => f.id === fieldId);
    if (!currentField?.conditional) {
      return isFieldVisible(fieldId); // fallback to original
    }

    const cond = currentField.conditional;
    const parentFieldId = cond.parentFieldId;
    if (!parentFieldId) return true;

    const parentValue = formData[parentFieldId];

    // Handle multi-value condition (new feature)
    if (Array.isArray(cond.values) && cond.values.length > 0) {
      if (Array.isArray(parentValue)) {
        // If parent is multi-select (array)
        return cond.values.some((v) => parentValue.includes(v));
      } else {
        // If parent is single value
        return cond.values.includes(String(parentValue || ""));
      }
    }

    // Fallback to old single value condition
    const triggerValue = cond.value;
    if (!triggerValue) return true;

    const actualParentValue = String(parentValue || "");

    const shouldShow = actualParentValue === triggerValue;
            ...
            <div className="flex-1 overflow-y-auto overscroll-contain px-4 py-4 sm:px-5 sm:py-5">
              <div className="max-w-5xl mx-auto">
                <FormBody
                  ...
                  toggleSubform={toggleSubform}
                  setErrors={setErrors}
                  isFieldVisible={isFieldVisibleWithMultiSupport}
                  isSectionVisible={isSectionVisible}
                  isViewOnly={isViewOnly}
                  isSectionReadOnly={isSectionReadOnly}
                  isFieldReadOnly={isFieldReadOnly}
