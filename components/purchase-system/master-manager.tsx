...
/**
 * Purchase Master — manage the dropdowns used across the procurement documents
 * (Supplier, Department, Payment Terms, …) and register new ones. Same UX as
 * the Inventory Master; bound to the purchase provider.
 */
...
import type { MasterOption, MasterType, PurchaseSubmoduleKey } from "@/lib/purchase-system/types";
...
const SUBMODULE_LABEL: Record<PurchaseSubmoduleKey, string> = {
  supplier: "Supplier",
  pr: "PR",
  sourcing: "RFQ",
  po: "PO",
...
export function MasterManager() {
  const { ready, masters, addMasterType } = usePurchase();
  const [activeKey, setActiveKey] = useState<string>("supplier_group");
  const [createOpen, setCreateOpen] = useState(false);

  // "supplier" is managed as a full entity (Supplier Master), not a raw
  // dropdown, so it's hidden here — its options are derived from that screen.
  const visibleMasters = useMemo(() => masters.filter((m) => m.key !== "supplier"), [masters]);
    ...
    <div className="h-full flex flex-col">
      <div className="px-4 sm:px-6 py-4 border-b shrink-0 flex items-start gap-3">
        <div className="min-w-0 flex-1">
          <h1 className="text-lg font-semibold tracking-tight">Purchase Master</h1>
          <p className="text-sm text-muted-foreground">
            Manage the dropdowns used across the procurement documents — Supplier, Department,
            Payment Terms and more. You can add your own too.
          </p>
        </div>
...
function MasterOptionsPanel({ master }: { master: MasterType }) {
  const { addMasterOption, updateMasterOption, deleteMasterOption, deleteMasterType } = usePurchase();
    ...
    >
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Create a reusable dropdown (e.g. Tax Group, Incoterm, Currency). You can add its options
            right after.
          </DialogDescription>
        </DialogHeader>
