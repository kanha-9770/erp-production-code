...
/**
 * HR Master — one management surface for every editable HR dropdown.
 *
 * Left rail lists each master (Department, Designation, Shift Type, …); the
 * right panel manages that master's option list: add, inline rename, toggle
 * active, delete. Mirrors the Inventory Master UX. New masters can be created
 * from the "New master" button or seeded in lib/hr-master/schema.ts.
 */
...
import { HrMasterProvider, useHrMaster } from "@/lib/hr-master/store";
import type { HrMasterOption, HrMasterType } from "@/lib/hr-master/types";
...
export function HrMasterManager() {
  return (
    <HrMasterProvider>
      <HrMasterManagerInner />
    </HrMasterProvider>
  );
}

function HrMasterManagerInner() {
  const { ready, masters, addMasterType } = useHrMaster();
  const [activeKey, setActiveKey] = useState<string>("department");
  ...
  if (!ready) {
    return <div className="p-6 text-sm text-muted-foreground">Loading HR masters…</div>;
  }
    ...
    <div className="h-full flex flex-col">
      <div className="px-4 sm:px-6 py-4 border-b shrink-0 flex items-start gap-3">
        <div className="min-w-0 flex-1">
          <h1 className="text-lg font-semibold tracking-tight">HR Master</h1>
          <p className="text-sm text-muted-foreground">
            Manage every HR dropdown in one place — Department, Designation,
            Shift Type, Employment Type and more. These lists feed the Employee
            Master and HR forms everywhere, and you can add your own.
          </p>
        </div>
      ...
      <div className="flex-1 min-h-0 grid grid-cols-1 md:grid-cols-[260px_1fr]">
        ...
        <div className="overflow-y-auto">
          {active ? (
            <MasterOptionsPanel master={active} />
          ) : (
            <div className="p-6 text-sm text-muted-foreground">
              No masters yet — create one with “New master”.
            </div>
          )}
        </div>
      </div>
...
function MasterOptionsPanel({ master }: { master: HrMasterType }) {
  ...
  const [deleting, setDeleting] = useState<HrMasterOption | null>(null);
            ...
            {editingId === o.id ? (
              ...
              <div className="flex items-center gap-2">
                <Switch
                  checked={o.active}
                  onCheckedChange={(v) =>
                    void updateMasterOption(master.key, o.id, { active: v })
                  }
                  aria-label="Active"
                />
                <Button
      ...
      <AlertDialog open={!!deleting} onOpenChange={(o) => !o && setDeleting(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Removing this option won’t change records that already use it, but it
              will no longer be selectable. Consider toggling it inactive instead.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <AlertDialog open={deletingMaster} onOpenChange={setDeletingMaster}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This removes the entire dropdown and all {master.options.length} of its
              options. Fields that referenced it will keep their stored text but lose
              the dropdown. This cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
...
function CreateMasterDialog({
  ...
  return (
    ...
    >
      <DialogContent>
        <DialogHeader>
          <DialogTitle>New HR master dropdown</DialogTitle>
          <DialogDescription>
            Create a reusable dropdown (e.g. Grade, Cost Center, Notice Period). You
            can add its options right after.
          </DialogDescription>
        </DialogHeader>
