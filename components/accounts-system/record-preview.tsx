...
/** Read-only detail pane for a selected accounts document. */

import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Separator } from "@/components/ui/separator";
import { Pencil, Trash2, Loader2 } from "lucide-react";
import { formatDate, formatMoney, formatNumber, resolveStatus } from "@/lib/accounts-system/format";
...
function displayValue(field: FieldDef, record: AccountsRecord): React.ReactNode {
...
export function RecordPreview({
  ...
  // Hide the auto-filled Invoice Outstanding when no invoice was chosen — gate
  // on the SAME dependency the form uses (invoiceRef), not the amount itself, so
  // a zero-balance (fully-paid) invoice still shows ₹0 consistently in both.
  const isShown = (f: FieldDef) =>
    isVisible(f) && !f.formHidden && !(f.requiresOpenInvoice && !String(record.invoiceRef ?? "").trim());
