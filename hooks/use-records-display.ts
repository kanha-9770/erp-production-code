...
export function useRecordsDisplay({
  ...
  const [activeFieldFilters, setActiveFieldFilters] = React.useState<FieldFilter[]>([]);
  const [selectedFieldForAdvancedFilter, setSelectedFieldForAdvancedFilter] =
    React.useState<string | null>(null);
  const [columnSearchFieldId, setColumnSearchFieldId] = React.useState<string | null>(null);
  ...
  const [orderedFields, setOrderedFields] = React.useState<FormFieldWithSection[]>([]);
  ...
  const [comments, setComments] = React.useState<Map<string, Comment[]>>(new Map());
  ...
  React.useEffect(() => {
    if (comments.size > 0) {
      ...
      comments.forEach((value, key) => { serializable[key] = value; });
  ...
  React.useEffect(() => {
    ...
    if (saved) {
      try { setConditionalRules(JSON.parse(saved)); } catch { /* ignore */ }
    }
  }, []);

  React.useEffect(() => {
    if (conditionalRules.length > 0) {
      localStorage.setItem("table-conditional-rules", JSON.stringify(conditionalRules));
    } else {
  ...
  React.useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      const newWidth = Math.max(100, resizeStartWidth + (e.clientX - resizeStartX));
      setColumnWidths((prev) => { const m = new Map(prev); m.set(resizingColumn, newWidth); return m; });
    };
  ...
  const getRecordForms = React.useCallback((record: EnhancedFormRecord): string[] => {
  ...
  const canEditRecord = React.useCallback(
    (record: EnhancedFormRecord) => {
      if (isAdmin) return true;
      return getRecordForms(record).every((fId) => hasPermissionForForm(fId, "EDIT"));
    },
  ...
  const canDeleteRecord = React.useCallback(
    (record: EnhancedFormRecord) => {
      if (isAdmin) return true;
      return getRecordForms(record).every((fId) => hasPermissionForForm(fId, "DELETE"));
    },
    [getRecordForms, hasPermissionForForm, isAdmin],
  );
  ...
  const recalculateFormulasForRecord = React.useCallback(
    (record: EnhancedFormRecord, changedFieldIds: Set<string> = new Set()) => {
      ...
      formulaFieldsToProcess.forEach((formulaField) => {
        ...
        if (changedFieldIds.size > 0 && !Array.from(deps).some((d) => changedFieldIds.has(d))) return;
        ...
        try {
          ...
          extractFieldReferences(config.expression).forEach((refId) => {
            variables[refId] =
              (currentValues[refId] !== undefined && currentValues[refId] !== null && currentValues[refId] !== "")
                ? currentValues[refId]
          ...
          const storeChain = (val: any) => {
            try {
              ...
              if (formulaField.originalId) runningValues[formulaField.originalId] = cv;
              if (formulaField.label) runningValues[formulaField.label] = cv;
            } catch { runningValues[formulaField.id] = result.value; }
          };
          ...
          if (idx !== -1) {
            ...
            newProcessed.push({
              fieldId: formulaField.id, fieldLabel: formulaField.label,
              fieldType: "formula", value: finalValue, displayValue: String(finalValue),
              order: formulaField.order ?? 999,
              sectionId: formulaField.sectionId || "default",
              sectionTitle: formulaField.sectionTitle || "General",
              subformId: formulaField.subformId, subformTitle: formulaField.subformTitle,
              formId: formulaField.formId || "", formName: formulaField.formName || "",
              lookup: {}, options: [], fieldDefinitions: [], icon: "",
            });
            affected.add(formulaField.id);
            storeChain(finalValue);
          }
      ...
      return { updatedProcessedData: newProcessed, affectedFormulaFields: affected };
    },
  ...
  const getUniqueFieldDefinitions = React.useCallback(
    (recs: EnhancedFormRecord[], isMerged: boolean, selectedFormFilter: string) => {
      ...
      recs.forEach((record) => {
        record.processedData.forEach((pd) => {
          ...
          if (!isMerged && pd.formId !== selectedFormFilter && record.formId !== selectedFormFilter) return;
          if (!fieldMap.has(key)) {
            fieldMap.set(key, {
              id: pd.fieldId, originalId: pd.fieldId, label: pd.fieldLabel,
              type: pd.fieldType, order: pd.order,
              sectionTitle: pd.sectionTitle || "General", sectionId: pd.sectionId || "default",
              subformId: pd.subformId, subformTitle: pd.subformTitle,
              formId: pd.formId || record.formId, formName: pd.formName || record.formName || "",
            });
          }
        });
      });
      ...
      return [...records].sort((a, b) => {
        ...
        if (recordSortField === "submittedAt") {
          ...
          if (recordSortField.includes("_")) {
            ...
            targetFormId = parts[0]; targetFieldId = parts[1];
          } else { targetFieldId = recordSortField; }
          const fd = (r: EnhancedFormRecord) =>
            targetFormId
              ? r.processedData.find((pd) => (pd.formId || r.formId) === targetFormId && pd.fieldId === targetFieldId)
  ...
  const getConditionalStyle = React.useCallback(
    (fieldDef: FormFieldWithSection, value: any, _displayText: string): React.CSSProperties => {
      const isDateField = ["date", "datetime"].includes(fieldDef.type);
      const isFormulaDate =
        fieldDef.type === "formula" && typeof value === "string" && /^\d{4}-\d{2}-\d{2}/.test(value);
        ...
        const matchingRule = conditionalRules
          ...
          .find((rule) => {
            ...
            switch (rule.condition) {
              ...
              case "greaterThan": return !isNaN(Number(value)) && !isNaN(Number(rule.value)) && Number(value) > Number(rule.value);
              case "lessThan": return !isNaN(Number(value)) && !isNaN(Number(rule.value)) && Number(value) < Number(rule.value);
      ...
      const matchingDateRule = conditionalRules
        .filter((r) =>
          ...
          ["today", "overdue", "dueSoon", "pastDue", "thisWeek", "nextWeek"].includes(r.condition),
        )
        .find((rule) => {
          switch (rule.condition) {
            case "today": return daysDiff === 0;
            case "overdue": return daysDiff < 0;
            case "dueSoon": return daysDiff >= 0 && daysDiff <= 7;
            case "pastDue": return daysDiff < -7;
            case "thisWeek": {
            ...
            case "nextWeek": {
              const sonw = new Date(today); sonw.setDate(today.getDate() - today.getDay() + 7);
              const eonw = new Date(sonw); eonw.setDate(sonw.getDate() + 6);
  ...
  const handleResizeStart = (e: React.MouseEvent, fieldId: string, currentWidth: number) => {
    e.preventDefault(); e.stopPropagation();
    setResizingColumn(fieldId); setResizeStartX(e.clientX); setResizeStartWidth(currentWidth);
  };
  ...
  const handleOpenDeleteConfirm = (record: EnhancedFormRecord) => {
    if (canDeleteRecord(record)) { setRecordToDelete(record); setDeleteConfirmOpen(true); }
  };
  ...
  const populatedRecordsWithPending = useMemo(() => {
    return formRecords.map((record) => {
      ...
      pendingChanges.forEach((change, changeKey) => {
        ...
        if (pdIndex !== -1) {
          updatedProcessed[pdIndex] = {
            ...updatedProcessed[pdIndex],
            value: change.value,
            displayValue: typeof change.value === "number" ? change.value.toLocaleString() : String(change.value ?? "—"),
          };
        }
      });
      ...
      if (hasPending) {
        const { updatedProcessedData } = recalculateFormulasForRecord(enhanced, new Set());
  ...
  React.useEffect(() => {
    const uniqueFields = getUniqueFieldDefinitions(baseRecords, isMergedMode, currentFormId);
    setOrderedFields((prev) => {
      if (prev.length !== uniqueFields.length) return uniqueFields;
      const prevIds = prev.map((f) => f.id).sort().join(",");
      const newIds = uniqueFields.map((f) => f.id).sort().join(",");
  ...
  React.useEffect(() => {
    const calc = () => {
      ...
      const maxRows = Math.floor((tableContainerRef.current.clientHeight - 40) / 36);
  ...
  const hierarchyGroups = useMemo(() => {
    ...
    fieldsToGroup.forEach((field) => {
      ...
      if (field.subformId) {
        ...
        if (!sfg) { sfg = { id: field.subformId, name: field.subformTitle || "Subform", sections: [] }; fg.subforms.push(sfg); }
        let sec = sfg.sections.find((s) => s.id === field.sectionId);
        if (!sec) {
          sec = { id: field.sectionId, title: field.sectionTitle !== "Default Section" ? field.sectionTitle : undefined, fields: [] };
  ...
  const filteredRecords = useMemo(() => {
    ...
    if (recordSearchQuery) {
      ...
      records = records.filter((r) =>
        r.processedData.some((pd) => (pd.displayValue ?? "").toString().toLowerCase().includes(q)),
      );
    }
  ...
  }, [baseRecords, recordSearchQuery, columnSearchFieldId, columnSearchValue, formFieldsWithSections, getFieldData, applyFieldFilters, sortRecords]);
