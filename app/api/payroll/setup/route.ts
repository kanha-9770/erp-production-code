...
function extractSetup(mappings: any): PayrollSetup {
  if (mappings && typeof mappings === 'object' && mappings._meta === META_KEY) {
    return {
      employee: { ...EMPTY_SETUP.employee, ...(mappings.employee || {}) },
      checkIn: { ...EMPTY_SETUP.checkIn, ...(mappings.checkIn || {}) },
      checkOut: { ...EMPTY_SETUP.checkOut, ...(mappings.checkOut || {}) },
    };
  }
  return EMPTY_SETUP;
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!setup.employee.fields.salary) errors.push('Map the Salary field on the Employee Profile form');
