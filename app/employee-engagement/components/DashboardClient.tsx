...
import { Target, Lightbulb, AlertCircle, TrendingUp, MessageSquare, Filter, Search, Calendar as CalendarIcon, Clock, Award, Save, RotateCcw, ShieldCheck, CheckCircle2, XCircle, HelpCircle, FileText, Printer, Download, Lock, Eye, Trophy, Sparkles, Star, Crown, Layers } from "lucide-react";
...
function SummaryStat({
  ...
  return (
    <div className={`rounded-md border px-4 py-3.5 ${tones[tone]}`}>
...
// Per-module signature colors. Kaizen uses the same amber as the
// "Best Kaizen" badges + Toyota-Production-System lineage, so every
// Kaizen-related chip / icon / highlight reads as one family. The other
// four modules use semantically aligned hues (blue = suggestion / idea,
// rose = problem, emerald = initiative / growth, violet = target / goal).
const MODULE_COLORS: Record<string, string> = {
  Kaizen: "#f59e0b",      // amber-500
  Suggestion: "#3b82f6",  // blue-500
  Problem: "#f43f5e",     // rose-500
  Initiative: "#10b981",  // emerald-500
...
export default function DashboardClient({
  ...
  const [activeTab, setActiveTab] = useState<"queue" | "leaderboard" | "reports">("queue");
  ...
  // Leaderboard-tab-only module slicer. Lets reviewers see the ranking
  // for a single module (Kaizen / Suggestion / Problem / Initiative /
  // Target) without disturbing the queue-tab module filter above.
  const [leaderboardModule, setLeaderboardModule] = useState<"All" | "Kaizen" | "Suggestion" | "Problem" | "Initiative" | "Target">("All");
  ...
  const employeePoints = useMemo(() => {
    ...
    >();
    const source =
      leaderboardModule === "All"
        ? filteredData
        : filteredData.filter((d) => d.moduleType === leaderboardModule);
  ...
  }, [filteredData, pointsBySubmission, bonusBySubmission, leaderboardModule]);

  // Per-module submission counts across the currently-filtered view,
  // used to label each pill in the Leaderboard module selector.
  const leaderboardModuleCounts = useMemo(() => {
  ...
  const exportReportCSV = () => {
    // Escape every field through a single helper so commas, quotes,
    // newlines and non-ASCII employee names survive Excel + Sheets.
    const esc = (v: unknown): string => {
      if (v === null || v === undefined) return "";
      const s = String(v);
      // Always quote — keeps numeric columns aligned and is harmless.
      return `"${s.replace(/"/g, '""')}"`;
    };
    ...
    // ── Report header ──────────────────────────────────────────────
    lines.push(row("Nessco — Employee Engagement Contribution Report"));
    lines.push(row("Period", reportRange.label));
    lines.push(
      row(
        "Date Range",
        `${reportRange.from.toLocaleDateString()} to ${new Date(reportRange.to.getTime() - 1).toLocaleDateString()}`,
      ),
    );
    lines.push(row("Generated", new Date().toLocaleString()));
    lines.push(row("Generated By", currentUserName));
    lines.push(blank());

    // ── Summary statistics ────────────────────────────────────────
    lines.push(row("SUMMARY"));
    lines.push(row("Metric", "Count"));
    lines.push(row("Total Submissions", reportSummary.total));
    lines.push(row("Approved", reportSummary.approved));
    lines.push(row("Rejected", reportSummary.rejected));
    lines.push(row("Needs Info", reportSummary.needsInfo));
    lines.push(row("Pending Review", reportSummary.pending));
    lines.push(row("Total Points Awarded", reportSummary.totalPoints));
    lines.push(blank());

    // ── Module breakdown ──────────────────────────────────────────
    lines.push(row("MODULE BREAKDOWN"));
    lines.push(row("Module", "Submissions"));
    for (const m of ["Kaizen", "Suggestion", "Problem", "Initiative", "Target"]) {
      lines.push(row(m, reportSummary.moduleCount[m] || 0));
    }
    lines.push(blank());

    // ── Top contributors (full list, not capped) ─────────────────
    lines.push(row("TOP CONTRIBUTORS"));
    lines.push(
      row("Rank", "Employee ID", "Employee Name", "Department", "Submissions", "Approved", "Rejected", "Total Points"),
    );
    reportSummary.topContributors.forEach((c, i) => {
      lines.push(
        row(i + 1, c.employeeId, c.employeeName, c.department, c.submissions, c.approved, c.rejected, c.points),
      );
    });
    lines.push(blank());

    // ── Detailed submissions ─────────────────────────────────────
    // The reviewer's table view condensed; one row per submission with
    // points, bonus, review status, reviewer, remark and best-Kaizen flag.
    ...
    // Sort the same way the on-screen Award Points table does by default
    // (earliest first) so the spreadsheet matches what reviewers see.
    const detailRows = [...reportItems].sort(
    ...
    for (const it of detailRows) {
      ...
      const rev = reviewBySubmission[it.id];
      const reviewLabel = rev ? REVIEW_META[rev.status].label : "Pending";
      lines.push(
        row(
          it.displayId || it.id.substring(0, 8).toUpperCase(),
          ...
          it.employeeName,
          it.department,
          getStatusMeta(it.status).label,
          reviewLabel,
          rev?.reviewer ?? "",
          rev?.reviewedAt ? new Date(rev.reviewedAt).toLocaleString() : "",
          typeof pts === "number" ? pts : "",
          bonus && bonus.points > 0 ? bonus.points : "",
          bonus?.reason ?? "",
          it.moduleType === "Kaizen" ? (bestKaizenBySubmission[it.id] ? "Yes" : "No") : "",
          remarkBySubmission[it.id] ?? "",
          rev?.notes ?? "",
        ),
      );
    }

    // CRLF + BOM keep Excel happy with Unicode names and stop the
    // whole file from collapsing into a single line on Windows builds.
    const csv = "﻿" + lines.join("\r\n") + "\r\n";
    const blob = new Blob([csv], { type: "text/csv;charset=utf-8;" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    const safeLabel = reportRange.label.replace(/[^\w]+/g, "-").replace(/^-|-$/g, "");
    const stamp = format(new Date(), "yyyyMMdd-HHmm");
    a.href = url;
    a.download = `engagement-report-${safeLabel}-${stamp}.csv`;
  ...
  return (
    <div className="flex-1 space-y-8 p-4 sm:p-6 lg:p-8 pb-12">

      
      {/* ── Structural layout change: Banner & Header Container ── */}
      <div className="rounded-xl border bg-card shadow-sm border-l-4 border-l-primary">
        <div className="px-4 sm:px-6 py-5 flex flex-wrap items-center justify-between gap-x-4 gap-y-3">
          ...
          <div className="flex items-center gap-3 text-xs text-muted-foreground">
            <div className="border-r pr-3">
              Reviewed: <span className="font-semibold text-foreground">{totalSubmissions > 0 ? Math.round(((reviewStats.approved + reviewStats.rejected + reviewStats.needsInfo) / totalSubmissions) * 100) : 0}%</span>
            </div>
            <div>
              Total Score: <span className="font-semibold text-primary">{Object.values(pointsBySubmission).reduce((a, b) => a + b, 0)}</span>
            </div>
          </div>
        </div>
      </div>

      {/* ── Premium Tabbed Navigation Bar (Completely Redesigns Layout Structure) ── */}
      <div className="flex border-b border-border">
        <button
          type="button"
          onClick={() => setActiveTab("queue")}
          className={`px-4 sm:px-6 py-3 text-sm font-semibold border-b-2 -mb-[2px] transition-all flex items-center gap-2 ${
            activeTab === "queue"
      ...
      <div className={activeTab === "queue" ? "space-y-8" : "hidden"}>
        {/* Floating Filter Hub */}
        <div className="rounded-xl border bg-card shadow-sm p-4 sm:p-5">
          ...
          {timeFilter === "custom" && (
            <div className="flex flex-wrap items-center gap-2.5 border-t pt-4 mt-4">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-6">
          <Card className="col-span-1 border-primary/20 bg-primary/5">
            <CardContent className="pt-6 pb-5">
              ...
              <p className="text-xs text-muted-foreground mt-1.5 font-medium">Total Submissions</p>
            </CardContent>
          </Card>
          ...
          {["Kaizen", "Suggestion", "Problem", "Initiative", "Target"].map((mod) => {
            ...
            return (
              <Card key={mod} className="shadow-sm hover:shadow-md transition-shadow">
                <CardHeader className="flex flex-row items-center justify-between pb-2 pt-5 space-y-0">
        ...
        <Card className="shadow-sm border-primary/30">
          <CardHeader className="pb-5 pt-5 border-b bg-primary/5">
              ...
              <Table>
                <TableHeader className="bg-muted/50">
                  <TableRow>
                    ...
                    <TableHead className="text-center w-[150px]">
                      <div className="flex items-center justify-center gap-1">
                        <Award className="h-3.5 w-3.5 text-amber-605" />
                        Bonus (0–{BONUS_MAX})
                      </div>
                    </TableHead>
                      ...
                      <TableRow key={item.id} className="hover:bg-muted/50 transition-colors">
                        ...
                        <TableCell className="text-center">
                          <div className="flex items-center justify-center gap-1.5">
                            ...
                            />
                            {justSaved ? (
                              <Save className="h-3.5 w-3.5 text-emerald-600 animate-bounce" />
                            ) : (
      ...
      <div className={activeTab === "leaderboard" ? "space-y-8" : "hidden"}>
        <Card className="shadow-sm">
          <CardHeader className="pb-5 pt-5 border-b space-y-5">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div className="space-y-1.5">
                ...
                <CardDescription>
                  {leaderboardModule === "All"
                    ? "Aggregated totals across every module. This is the value shown on each employee's profile."
                    : `Ranking based only on ${leaderboardModule} submissions in the current view.`}
                </CardDescription>
              </div>
              <Badge variant="secondary" className="px-2.5 py-1">{employeePoints.length} employees</Badge>
            </div>

            {/* Module slicer — re-aggregates the ranking by module so HR
                can see the top Kaizen / Suggestion / Problem / Initiative
                / Target contributors independently. */}
            <div className="flex flex-wrap items-center gap-2">
              {(["All", "Kaizen", "Suggestion", "Problem", "Initiative", "Target"] as const).map((mod) => {
                const isActive = leaderboardModule === mod;
                const Icon = mod === "All" ? Layers : (MODULE_ICONS[mod] || Target);
                const color = mod === "All" ? undefined : MODULE_COLORS[mod];
                const count = leaderboardModuleCounts[mod] || 0;
                return (
                  <button
                    key={mod}
                    type="button"
                    onClick={() => setLeaderboardModule(mod)}
                    className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium border transition-all ${
                      isActive
                        ? "bg-primary text-primary-foreground border-primary shadow-sm"
                        : "bg-background text-foreground border-border hover:bg-muted"
                    }`}
                    style={isActive && color ? { backgroundColor: color, borderColor: color, color: "#fff" } : undefined}
                  >
                    <Icon className="h-3.5 w-3.5" style={!isActive && color ? { color } : undefined} />
                    <span>{mod === "All" ? "All Modules" : mod === "Initiative" ? "Self Initiative" : mod === "Target" ? "Self Target" : mod === "Problem" ? "Problem Reg." : mod}</span>
                  ...
                  return (
                    <TableRow key={e.employeeId} className="hover:bg-muted/50 transition-colors group">
                      <TableCell className="text-center text-sm font-semibold text-muted-foreground">
                        {rank === 1 ? "🏆 Rank #1" : rank === 2 ? "🥈 Rank #2" : rank === 3 ? "🥉 Rank #3" : `#${rank}`}
                      </TableCell>
                      <TableCell>
                  ...
                  <TableRow>
                    <TableCell colSpan={8} className="h-24 text-center text-muted-foreground">
                      {leaderboardModule === "All"
                        ...
                        : `No ${leaderboardModule} submissions in the current view.`}
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
      ...
      {/* ── TAB 3: AUDIT & REPORTS ── */}
      <div className={activeTab === "reports" ? "space-y-8" : "hidden"}>
        <Card className="shadow-sm print:shadow-none" id="engagement-report">
          <CardHeader className="pb-5 pt-5 border-b">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div className="space-y-1.5">
                <CardTitle className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                {["Kaizen", "Suggestion", "Problem", "Initiative", "Target"].map((m) => {
                  ...
                  return (
                    <div key={m} className="rounded-md border px-4 py-3 flex items-center gap-3">
                      <div
